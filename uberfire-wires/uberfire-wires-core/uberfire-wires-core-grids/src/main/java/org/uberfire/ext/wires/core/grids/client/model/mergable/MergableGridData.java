...
public class MergableGridData extends BaseGridData<MergableGridRow, MergableGridColumn<?>, MergableGridCell<?>> implements IMergableGridData {
    ...
    protected void indexColumn( final int columnIndex ) {
    ...
    public void updateColumn( final int index,
        ...
        final MergableGridColumn<?> existing = columns.get( index );
    ...
    @Override
    public void deleteColumn( final MergableGridColumn<?> column ) {
        final int index = column.getIndex();
        for ( MergableGridColumn<?> c : columns ) {
            if ( c.getIndex() > index ) {
        ...
        for ( MergableGridRow row : rows ) {
            ...
            final Map<Integer, MergableGridCell<?>> clone = new HashMap<Integer, MergableGridCell<?>>( row.getCells() );
            for ( Map.Entry<Integer, MergableGridCell<?>> e : clone.entrySet() ) {
                if ( e.getKey() > index ) {
